...
def generate_portfolio_decision(ticker: str, tech_analysis: str, fund_analysis: str, persona: str = "Balanced") -> str:
    """
    ...
    template = """
    You are an elite Portfolio Manager Agent operating under a {persona} investment style. Your task is to review the reports from your 
    ...
    trends against the fundamental catalysts, tailored to a {persona} approach. Ensure you explicitly say "Recommendation: BUY", 
    "Recommendation: SELL", or "Recommendation: HOLD".
    """
    
    prompt = PromptTemplate(
        template=template, 
        input_variables=["ticker", "tech_analysis", "fund_analysis", "persona"]
    )
    chain = prompt | llm
